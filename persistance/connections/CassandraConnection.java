...
import com.airvana.loadtool.client.SimuPool;
...
public final class CassandraConnection {
	...
	protected CassandraConnection() {
		CASSANDRA_INFO = loadCassandraConfig();
	...
	/**
	 * Loads the cassandra-config.properties. <b>Deprecated</b> as the
	 * cassandra-config.properties is no more used and the values are fetched
	 * from GUI of LoadTool.
	 * 
	 ...
	 * @return cassandra config information inform of Map.
	 */
	private Map<String, String> loadCassandraConfig() {
		return SimuPool.getReference().getDeviceInfo().getPersistenceConfig()
				.getPersistenceInfo();
	}
	...
	/**
	 * Closes the {@link Cluster} which has been opened for Connection.
	 * 
	 * @return {@link CloseFuture} after performing close on Cluster, if no
	 *         cluster is available, {@code null} value will be returned.
	 */
	protected final CloseFuture closeCluster() {
		if (cluster != null) {
	...
	/**
	 * Logs the information about the connection made.
	 */
	private void logConnectionInfo() {
		Metadata metadata = cluster.getMetadata();
		LOG.info("Cassandra Connected Successfully. Below mentioned are the details of connection made.");
		LOG.info("Connected to cluster: " + metadata.getClusterName());
		for (Host host : metadata.getAllHosts()) {
			LOG.info("Datacenter: " + host.getDatacenter() + "; Host: "
					+ host.getAddress() + "; Rack: " + host.getRack() + ";");
		}
		LOG.info("===========================END of Connection Info=====================================");
	}
}
